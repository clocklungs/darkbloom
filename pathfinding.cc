...
//    empty(); /*empty stack before using*/
    ...
    if((x==findx && y == findy)||tmp.f==INFINITE)
      {/*done*/
      done=1;
      }
    }/*end while*/
...
/*  printf("DEBUG: start path_find\n");*/
  for(h=0;h<WORLDHEIGHTMAX;h++)
    {/*inits world*/
    for(w=0;w<WORLDWIDTHMAX;w++)
      {
      if(world.world[h][w]%2==0)
       {/*block avaibale for moving into*/
        path[h][w].closed=-1;
        path[h][w].g=0;
        path[h][w].h=-1;
        path[h][w].f=-1;
...
printf("DEBUG: funct\n");*/
  ...
  while(search != NULL)
  {
    ...
    if((search->area == world.name) && (search->object->active) &&
       (search->object->collidable != NONE))
    {
...
//    printf("no path\n");
    ...
    while(!done)
      ...
      if(tmpx==object.x && tmpy == object.y)
        {/*checks if it found the end point*/
        done = 1;
        }
...
void closest_path(Object &object, int findx, int findy)
  ...
  for(int i=0;i<len;i++)
    {/*checks for lowest wieght value*/
    if(open_slot[i].h<tmp.h)
      {/*found new lowest wieght*/
      tmp.h=open_slot[i].h;
      tmp.x=open_slot[i].x;
      tmp.y=open_slot[i].y;
      }
