...
void sidebarUpdate(bool force) {
    ...
    for (i = 0; i < 3; ++i) {
        ...
        /*draw hp stats*/
        snprintf(buffer, SBARTEXTMAX + 2, "%.3i/%.3i HP", status[i].hp, status[i].max_hp);
        buffer[SBARTEXTMAX + 2] = '\0';
        rightFont(font, screen, screen->w - 3, dest.y + SBARIMAGEH, buffer);
        /*draw mp stats*/

        snprintf(buffer, SBARTEXTMAX + 2, "%.3i/%.3i MP", status[i].mp, status[i].max_mp);
        buffer[SBARTEXTMAX + 2] = '\0';
        ...
        if (strlen(status[i].weapon) && sidebar_vars.toString(status[i].weapon, buffer, SBARBUFFERMAX) != NULL) {
            image = IMG_Load(buffer);
            if (image == NULL) {
