...
int processEval(char *string, FILE *script, const char *filename, int &linenum, char line[LINESIZE]);
...
bool runScript(const char *filename, Object *who, Object *what) {
    ...
    while (!done && !feof(script)) {
        ...
        /*set player position*/
        case 'P':
            if (sscanf(interpolateVars(&line[1], script_vars), "%i%i", &player.x, &player.y) < 2) {
                fprintf(stderr, "%s:%i: error: ", filename, linenum);
        ...
        case 'D':
            ...
            if (temp < 0)
                ...
                script_vars.set("DIALOG_RETVAL", "int", &temp);
            break;
        /*delay*/
        case 'd':
            if (!sscanf(interpolateVars(&line[1], script_vars), "%i", &temp)) {
        ...
        case 'W':
            if (who == NULL)
                break;
            if (sscanf(interpolateVars(&line[1], dialog_vars), "%80s = %80s", property, value) < 2) {
                fprintf(stderr, "%s:%i: error: ", filename, linenum);
...
bool runScript(Object &who, Object &what, Why why) {
    ...
    if (why == ACTIVATION) {
        //    printf("A_SCRIPT: %s\n", what.a_script.c_str());
        if (!what.a_script.length())
            return true;
        return runScript(what.a_script.c_str(), &who, &what);
    }

    //  printf("C_SCRIPT: %s\n", what.c_script.c_str());
    if (!what.c_script.length())
        return true;
