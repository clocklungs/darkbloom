...
int mainMenu() {
    ...
    static int curr = 0;
    const char *text[] = {"New game", "Continue", "Options", "Quit", "END"};
    Button menuItem[4];
    ...
    while (SDL_WaitEvent(&event)) {
        ...
        if (event.type == SDL_KEYUP) {
            ...
            default:
                ...
                if (checkToggles(event.key.keysym.scancode) == 3)
                    return 1;
                break;
...
int optionMenu(int curr) {
    ...
    int i, top, left, items, width, prev, selected;
    SDL_Keycode key;
    const char *text[] = {"Fullscreen: ", "Sounds: ", "Music: ", "END"};
    Button menuItem[3];
    int *values[3];
    ...
    while (SDL_WaitEvent(&event)) {
        ...
        if (event.type == SDL_KEYUP) {
            ...
            default:
                ...
                if (checkToggles(event.key.keysym.scancode))
                    return curr;
                break;
            } /*endof switch(event.key.keysym.sym)*/
        }
