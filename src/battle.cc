...
void battlefield_init(int nmy, int boss) {
    ...
    for (i = 0; i < MAGICS; i++) { /*laods magic*/
    ...
    {                              /*loads current players*/
        if (status[i].active == true) {
            ...
            if (i == 0) {
                ...
                battle_players[i].wrect.y = battle_players[i - 1].wrect.y + (battle_players[i - 1].plyr_rect.h);
            }
        }
    }
    /*HARDCODE*/
    ...
    {                         /*Loads enemies into battle*/
        ...
        battle_enemies[i].image[0] = IMG_Load(enemies[random_enemy].graphic_idle.c_str());
        battle_enemies[i].image[1] = IMG_Load(enemies[random_enemy].graphic_attack.c_str());
    ...
    {              /*loads a boss if there is one*/
        ...
        battle_enemies[i].image[0] = IMG_Load(::boss.graphic_idle.c_str());
        battle_enemies[i].image[1] = IMG_Load(::boss.graphic_attack.c_str());
    }
    cursor.image = IMG_Load("images/bcursor.png");
    cursor.selected = 0;
    cursor.rect.w = CURSORW;
    cursor.rect.h = CURSORH;
    cursor.rect.x = battle_enemies[cursor.selected].wrect.x + (battle_enemies[cursor.selected].nmy_rect.w / 2);
    ...
    pcursor.rect.x = battle_enemies[pcursor.selected].wrect.x + (battle_enemies[pcursor.selected].nmy_rect.w / 2);
...
void battle_destruct() { /*frees up images that are no longer needed*/
    int i;
...
bool battleloop(int nmy, int boss) { /*the main loop true true survied battle false died*/
    SDL_Event event;
    ...
    while ((SDL_WaitEvent(&event))) { /*main even loop*/
        enemy_turn++;

        for (int go = 0; go < nmy; go++) {
            if (enemy_turn % battle_enemies[go].agility == 0 && battle_enemies[go].dead == 1) { /*determines when normal enemies will go*/
        ...
        for (int go = 0; go < boss; go++) { /*boss when normal enemies will go*/
        ...
        case SDL_KEYDOWN: /*registars key down*/
            if (keys[SDL_SCANCODE_DOWN]) {
                ...
                if (selected_menu_choice == 1 && menu_level == 0) {
                    ...
                    menu_level = pattacking(pcursor.selected, cursor.selected, selected_menu_choice, menu_level);
                    select_enemy = 1;
                } else {
                    select_enemy = 0;
                }
            } else if (keys[SDL_SCANCODE_SPACE]) { /*FIX*/
                if (menu_level == 1) {
                    menu_level = 0;
                } else {
                    select_enemy = 1;
        ...
        if (enemy_dead == currenemies) { /*giving exp and leveling really should be in it's own funciton*/
            int tmpexp;
        ...
        enemy_dead = 0;
        for (int z = 0; z < currplayers; z++) { /*checks if players are alive*/
...
bool blitscreen(int select_enemy, int selected_menu_choice) { /*blits the main images to the screen excpet fighting peeople*/
    int i, tmpdead;
    SDL_Rect dest;
    tmpdead = 0;
    ...
    for (i = 0; i < currplayers; i++) /*FIX: change to current amount of plyrs*/
    {
        if (battle_players[i].attacking == 1) {
            ...
            SDL_BlitSurface(battle_players[i].image[0], NULL, screen, &battle_players[i].wrect);
        } else if (status[i].status == DEAD) {
            tmpdead++;
        }
    }
    ...
    for (i = 0; i < currenemies; i++) {
        if (battle_enemies[i].dead == 0) {
            ...
            SDL_BlitSurface(battle_enemies[i].image[0], NULL, screen, &dest);
        }
    }
    if (select_enemy == 0) {
...
int pattacking(int player, int enemy, int selected_menu_choice, int menu_level) { /*player attacks*/
    int i, changeframe;
    SDL_Rect tmp;
    char hit[DMGNUMLEN];
    ...
    distancex = ((battle_players[player].wrect.x - (battle_enemies[enemy].wrect.x + battle_enemies[enemy].nmy_rect.w)) / ATTACKLOOP);
    distancey = (battle_players[player].wrect.y - battle_enemies[enemy].wrect.y) / ATTACKLOOP;
    if (selected_menu_choice == 0 && menu_level == 0) {
        for (i = 0; i < ATTACKLOOP; i++) { /*move to enemy*/
            blitscreen(0, 0);
            SDL_BlitSurface(battle_players[player].image[0], NULL, screen, &tmp);
            updateDisplay();
            delay(10, false);
            tmp.x -= distancex;
            tmp.y -= distancey;
        ...
        writeFont(&highlight, screen, (battle_enemies[enemy].nmy_rect.w + battle_enemies[enemy].wrect.x), battle_enemies[enemy].wrect.y, hit);
        updateDisplay();
        delay(ATTACKDEL, false);

        for (i = 0; i < ATTACKLOOP; i++) { /*move back*/
            blitscreen(0, 0);
            SDL_BlitSurface(battle_players[player].image[0], NULL, screen, &tmp);
            updateDisplay();
            delay(10, false);
    ...
    } else if (menu_level == 1 && status[player].mp > 20) { /*casts spells if magic above 20*/
        distancex = ((battle_players[player].wrect.x - battle_enemies[enemy].wrect.x) / MAGICLOOP);
        distancey = (battle_players[player].wrect.y - battle_enemies[enemy].wrect.y) / MAGICLOOP;
        changeframe = MAGICLOOP / MAGICFRAMES;

        for (i = 0; i < MAGICLOOP; i++) { /*move to enemy*/
            blitscreen(0, 0);
            if (i % changeframe == 0) {
                spell[selected_menu_choice].spell.x += 85;
            }
            SDL_BlitSurface(battle_players[player].image[2], NULL, screen, &battle_players[player].wrect);
            SDL_BlitSurface(spell[selected_menu_choice].magic, &spell[selected_menu_choice].spell, screen, &tmp);
            updateDisplay();
...
void battlemenu(int menu_level) { /*loads the battle menu*/
    int i;
    if (menu_level == 0) {
...
void playerturn(int selected) { /*set the pcursor[player cursor*/
    ...
    pcursor.rect.x = battle_players[pcursor.selected].wrect.x + (battle_players[pcursor.selected].plyr_rect.w / 2);
...
int bmenuselect(int selected, int menu_level) {
    ...
    if (menu_level == 0) {
        writeFont(&highlight, battle.bmenu0, 3, selected * BMENUOFFSET, bchoice0[selected]);
        SDL_BlitSurface(battle.bmenu0, NULL, screen, &battle.bmenu_rect);
    } else if (menu_level == 1) {

        writeFont(&highlight, battle.bmenu1, 3, selected * BMENUOFFSET, bchoice1[selected]);
        SDL_BlitSurface(battle.bmenu1, NULL, screen, &battle.bmenu_rect);
    }
    return selected;
} /*int bmenuselect(int selected)*/

void attack_cursor_l() { /*moves attack cursor to the left*/
    cursor.selected--;
...
void attack_cursor_r() { /*moves attack cursor to the right*/
    cursor.selected++;
...
void eattacking(int player, int enemy, int selected_menu_choice) { /*this enables the enemy to attack the player*/
    int i;
    SDL_Rect tmp, dest;
    char hit[DMGNUMLEN];
    int distancex, distancey;
    ...
    distancex = (((battle_enemies[enemy].wrect.x + battle_enemies[enemy].nmy_rect.w) - battle_players[player].wrect.x) / ATTACKLOOP);
    distancey = (battle_enemies[enemy].wrect.y - battle_players[player].wrect.y) / ATTACKLOOP;

    for (i = 0; i < ATTACKLOOP; i++) { /*move to player*/
        blitscreen(0, 0);
        dest.x = tmp.x + battle_enemies[enemy].offset_x;
        dest.y = tmp.y + battle_enemies[enemy].offset_y;
        SDL_BlitSurface(battle_enemies[enemy].image[0], NULL, screen, &dest); /*FIX: enemy images*/
        updateDisplay();
        delay(10, false);
        tmp.x -= distancex;
        tmp.y -= distancey;
    ...
    if (status[player].hp < 1) {
        ...
        if (player > 2) { /*if player dead puts players cursor on living player*/
            player = 0;
        }
        if (status[player].status == DEAD) { /*looking for that living player*/
        ...
        if (status[player].status == DEAD) { /*if it was still unable to find that living player*/
            for (int y = 0; y < MAXPLYRS; y++) {
                if (status[y].status != DEAD) {
                    player = y;
                    break;
    ...
    writeFont(&highlight, screen, battle_players[player].wrect.x, battle_players[player].wrect.y, hit);
    sidebarUpdate();
    updateDisplay();
    delay(ATTACKDEL, false);
    ...
    for (i = 0; i < ATTACKLOOP; i++) { /*move back*/
        ...
        dest.x = tmp.x + battle_enemies[enemy].offset_x;
        dest.y = tmp.y + battle_enemies[enemy].offset_y;
        SDL_BlitSurface(battle_enemies[enemy].image[0], NULL, screen, &dest);
        updateDisplay();
        delay(10, false);
        tmp.x += distancex;
        tmp.y += distancey;
...
int hitfor(int a) { /*Ed's function to determine how much a person hits for*/
    int r1, r2;
    float poo;
    poo = a;
