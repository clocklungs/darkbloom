...
bool Object::load(const char *filename)
{
  ...
  while(true)
  {
    ...
    line++;
    a_script = buffer;
    if(a_script == "NONE") a_script = "";

    if(fscanf(in, "c_script: %99s\n", buffer) != 1) break;
    line++;
    c_script = buffer;
    if(c_script == "NONE") c_script = "";

    if(fscanf(in, "collidable: %99s\n", buffer) != 1) break;
    if(!strcmp(buffer, "NONE")) collidable = NONE;
    else if(!strcmp(buffer, "FULL")) collidable = FULL;
      else if(!strcmp(buffer, "MOVE")) collidable = MOVE;
        else break;
    line++;
...
Object* Object::up(bool move)
{
  ...
  if((y-1 < 0 || y >= world.h) || (x < 0 || x >= world.w)) return NULL;

  /*check if there is a solid tile in the way*/
  if(world.world[y-1][x] % 2) return NULL;
...
Object* Object::down(bool move)
{
  ...
  /*check if x, y coord will be inside of map area*/
  if((y < 0 || y+1 >= world.h) || (x < 0 || x >= world.w)) return NULL;
...
Object* Object::left(bool move)
{
  ...
  /*check if x, y coord will be inside of map area*/
  if((y < 0 || y >= world.h) || (x-1 < 0 || x >= world.w)) return NULL;

  /*check if there is a solid tile in the way*/
  if(world.world[y][x-1] % 2) return NULL;
...
Object* Object::right(bool move)
{
  ...
  /*check if x, y coord will be inside of map area*/
  if((y < 0 || y >= world.h) || (x < 0 || x+1 >= world.w)) return NULL;
...
bool AnimatedObject::load(const char *filename)
{
  ...
  while(true)
  {
    ...
    if(fscanf(in, "move_frame_start: %i\n",
              &move_frame_start[RIGHT]) != 1) break;
    line++;
...
void AnimatedObject::update()
{
  ...
  if(step > 0)
  {
    ...
    if(moving)
    {
      ...
      if(elapsed >= ANIM_MOVE_INTERVAL)
      {
        elapsed = 0;
        frame++;
        if(frame >= last_frame[direction]) frame = move_frame_start[direction];
      }
    }
    else
    {
  ...
  if(frame >= last_frame[direction]) frame = 0;
}
...
ObjectList::~ObjectList()
{
  ...
  while(search != NULL)
  {
    ...
    if(temp->object != NULL)
    {
      if(temp->animated) delete (AnimatedObject*)temp->object;
      else delete temp->object;
      temp->object = NULL;
    }
    delete temp;
  }
...
Object* ObjectList::add(const char *filename, int y)
{
  ...
  if(node->object->persistent)
  {
    ...
    while(search != NULL)
    {
      ...
      if((search->object->name == node->object->name) &&
         (search->area == node->area))
      {
        printf("DEBUG: %s already exists in %s\n", search->object->name.c_str(),
               search->area.c_str());
        if(node->animated) delete (AnimatedObject*)node->object;
        else delete node->object;
        node->object = NULL;
  ...
  /*insert at front?*/
  if((head == NULL) ||
     (search == head && search->layer <= node->layer))
  {
...
void ObjectList::draw()
{
  ...
  while(search != NULL)
  {
    ...
    {
      if(search->animated) ((AnimatedObject*)search->object)->draw();
      else search->object->draw();
    }
    search = search->next;
  }
}
...
Object* ObjectList::collide(int x, int y, bool &move, Direction dir)
{
  ...
  while(search != NULL)
  {
    ...
    if(search->object->active && search->object->x == x &&
       search->object->y == y && search->area == world.name)
    {
      switch(search->object->collidable)
      {
        ...
        case MOVE:
          ...
          if(move)
          {
            ...
            if(oldx!=search->object->x || oldy!=search->object->y) move = true;
          }
          else return NULL;
        default:
          break;
...
void ObjectList::activate()
{
  ...
  while(search != NULL)
  {
    //printf("Object: %s - %s\n", search->object.name.c_str(),
    //       search->area.c_str());
    if(search->area == world.name) search->object->active = true;
    else search->object->active = false;
    search = search->next;
  }
}
...
void ObjectLayerList::add(ObjectList &list)
{
  ...
  while(search != NULL)
  {
    if(search->animated) add(*((AnimatedObject*)search->object));
    else add(*(search->object));
    search = search->next;
  }
}
