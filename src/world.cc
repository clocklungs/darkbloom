...
void World::createImage(const char *filename) {
    ...
    image = SDL_CreateRGBSurface(SDL_SWSURFACE, w * TILEWIDTH, h * TILEHEIGHT, screen->format->BitsPerPixel, screen->format->Rmask,
...
void World::load(const char *filename) {
    ...
    if ((fscanf(in, "%d%d", &w, &h) < 2) || (w < WWDISPLAY || h < WHDISPLAY) || (w > WORLDWIDTHMAX || h > WORLDHEIGHTMAX)) {
        fprintf(stderr, "malformed level file: %s: %i\n", filename, line);
    ...
    /*read in tile data*/
    for (wy = 0; wy < h; wy++) {
        for (wx = 0; wx < w; wx++) { /*creates game world from file*/
            do {
                c = fgetc(in);
                if (c == '\n') {
                    if (wx != 0) {
