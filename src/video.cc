...
void initVideo(const char *title) {
    ...
    window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1024, 768, SDL_WINDOW_RESIZABLE);
    if (window == NULL) {
    ...
    dbox_back = SDL_CreateRGBSurface(screen->flags, dbox->w, dbox->h, screen->format->BitsPerPixel, screen->format->Rmask, screen->format->Gmask,
...
void toggleFullscreen() {
    ...
    save = SDL_CreateRGBSurface(screen->flags, screen->w, screen->h, screen->format->BitsPerPixel, screen->format->Rmask, screen->format->Gmask,
                                screen->format->Bmask, screen->format->Amask);
    if (save == NULL) {
