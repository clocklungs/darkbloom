...
void gameOver() { printf("STUB: GAME OVER\n"); }
...
void worldLoop() {
    ...
    while (true) {
        ...
        if (collide_object != NULL) {
            ...
            if (!runScript(player, *(collide_object), COLLISION))
                return;
            collide_object = NULL;
        }
        ...
        if (goodevent) { /*main even loop*/
            ...
            case SDL_KEYUP:
                ...
                case SDL_SCANCODE_KP_ENTER:
                    ...
                    if (active_object != NULL) {
                        ...
                        if (!runScript(player, *(active_object), ACTIVATION))
                            return;
                        active_object = NULL;
                    }
                    break;
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                if (event.button.x > 480 || event.button.y > 480)
                    break;
                /*it was, convert the button click to a tile x, y*/
                dest_x = (event.button.x + (world.viewable.x - world.viewport.x)) / TILEWIDTH;
                dest_y = (event.button.y + (world.viewable.y - world.viewport.y)) / TILEHEIGHT;
                /*clear old stack*/
