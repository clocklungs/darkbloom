...
int dboxChoice(const char *title, Choice choices[DBOX_CHOICES]) {
    ...
    for (i = 0; i < DBOX_CHOICES; ++i) {
        ...
        if (i == 0) {
            writeFont(&dboxHighlightFont, screen, x, y + (dboxFont.h * (i + 1)), choices[i].text.c_str());
        } else {
            writeFont(&dboxFont, screen, x, y + (dboxFont.h * (i + 1)), choices[i].text.c_str());
        }
        /*register button geometry for each choice*/
        choices[i].button.top = y + (dboxFont.h * (i + 1)), choices[i].button.left = x;
        choices[i].button.bottom = choices[i].button.top + dboxFont.h;
        choices[i].button.right = choices[i].button.left + (dboxFont.w * strlen(buffer));
    }
    ...
    while (SDL_WaitEvent(&event)) {
        ...
        if (curr != prev) {
            ...
            writeFont(&dboxFont, screen, x, y + ((prev + 1) * dboxFont.h), choices[prev].text.c_str());
            prev = curr;
            /*highlight new menu item*/
            writeFont(&dboxHighlightFont, screen, x, y + ((curr + 1) * dboxFont.h), choices[curr].text.c_str());
            /*update display*/
            updateDisplay();
        }
...
void dboxText() {
    ...
    for (i = 0; i < DBOX_MAXLINES; ++i) {
        writeFont(&dboxFont, screen, dbox_rect.x + DBOX_TEXTLEFT, dbox_rect.y + DBOX_TEXTTOP + (dboxFont.h * i), dbox_lines[i]);
    }
...
void dboxDraw() {
    ...
    if (config.sounds) {
        if (dialog_vars.eval("DBOXSND")) {
            ...
            if (var->type != VAR_STRING)
                ...
                dbox_snd = playFile(var->data.string_data->c_str(), dialog_vars.toInt("DBOXSNDLOOPS"));
            }
        }
    }
    ...
    if (var == NULL || var->type != VAR_STRING)
        dbox_img = NULL;
    else {
        dbox_img = IMG_Load(var->data.string_data->c_str());
        if (dbox_img == NULL) {
            fprintf(stderr, "error opening image '%s': %s", var->data.string_data->c_str(), SDL_GetError());
        }
    }
    ...
    /*draw title/caption*/
    writeFont(&dboxFont, screen, dbox_rect.x + DBOX_TITLELEFT, dbox_rect.y + DBOX_TITLETOP, title);
}
